...
const contractorNavigation = [
  ...
  { href: "/dashboard/inquiries/received", label: "Received Inquiries", mark: "04" },
];

const clientNavigation = [
  { href: "/dashboard/inquiries", label: "My Inquiries", mark: "01" },
];

/**
 * Shared placeholder shown while auth is resolving or a guard redirect is
 * in flight. Exported so dashboard pages reuse the exact same treatment
 * and the wrong-role UI never flashes.
 */
export function DashboardAccessPending({ label = "Loading your workspace" }: { label?: string }) {
  return (
...
export function DashboardShell({
  ...
  if (!auth.user) {
    // Auth resolved without a session: the guard effect in the page's
    // role hook has already requested the redirect to /login.
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#f4f1eb] text-[#183c31]">
        <div className="text-sm font-semibold text-[#607068]">
  ...
  return (
    <div className="min-h-screen bg-[#f4f1eb] text-[#1d2a25]">
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-72 flex-col border-r border-[#d7d8d1] bg-[#183c31] px-7 py-8 text-[#f4f1eb] lg:flex">
        ...
        <div className="mt-16">
          <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.24em] text-[#91aaa0]">
            Workspace
          </p>
          ...
          >
            {(isClient ? clientNavigation : contractorNavigation).map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center justify-between rounded-xl px-4 py-3 text-sm font-semibold transition ${
                    active
                      ? "bg-[#f4f1eb] text-[#183c31]"
                      : "text-[#c6d3cc] hover:bg-[#285847] hover:text-white"
                  }`}
                >
                  {item.label}
        ...
        <div className="mt-auto border-t border-[#44665a] pt-5">
          ...
          <p className="mt-1 text-xs text-[#91aaa0]">
            {isClient ? "Client account" : "Contractor account"}
          </p>
      ...
      <div className="lg:pl-72">
        ...
        <nav
          className="flex gap-2 overflow-x-auto border-b border-[#d7d8d1] px-5 py-3 lg:hidden sm:px-8"
